package CINE_v11_Compra1Entrada_JUANJO_ALBEROLA;
...
public class Cine {
	...
	public void compraEntradaPelicula() throws InterruptedException {
	    ...
	    if (pelicules.isEmpty()) {
	        System.out.println("No hi ha pel·lícules disponibles. Tornant al menú principal.");
	        return;
	    }
	    llistarPelicules();
	    ...
	    while (peliculaTriada == null) {
	        System.out.println("Per favor, elegeix una pel·licula [0 -> Cancel·lar acció]:");
	        try {
	            int eleccio = Integer.parseInt(teclat.next());
	            if (eleccio == 0) return;
	    ...
	    if (sessions.isEmpty()) {
	        System.out.println("No hi ha sessions disponibles per aquesta pel·lícula.");
	        return;
	    }
	    ...
	    while (sessioTriada == null) {
	        System.out.println("Per favor, elegeix una sessió [0 -> Cancel·lar acció]:");
	        try {
	            int eleccio = Integer.parseInt(teclat.next());
	            if (eleccio == 0) return;
	    ...
	    while (seientTriat == null) {
	        System.out.println("Tria un seient disponible [O], primer introdueix la fila [0 -> Cancel·lar acció]:");
	        try {
	            int fila = Integer.parseInt(teclat.next());
	            if (fila == 0) return;
	            if (fila > 0 && fila <= sessioTriada.getSeients().length) {
	                System.out.println("Introdueix el nombre del seient [0 -> Cancel·lar acció]:");
	                int seient = Integer.parseInt(teclat.next());
	                if (seient == 0) return; 
	                if (seient > 0 && seient <= sessioTriada.getSeients()[fila - 1].length) {
	                    ...
	                    if (!seientTriat.verificaSeient()) {
	                        System.out.println("El seient escollit no està disponible.");
	                        seientTriat = null;
	                    } else {
	                        seientTriat.reservaSeient();
	                    }
	                } else {
	                    System.out.println("Número de seient invàlid.");
	                }
	            } else {
	                System.out.println("Número de fila invàlid.");
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("No has introduit un valor correcte.");
	            teclat.nextLine();
	    ...
	    if (pagamentEntrada(sessioTriada.getPreu())) {
	        ...
	        System.out.println("Seient ocupat amb èxit.");
	        sessioTriada.imprimirTicket(seientTriat, sessioTriada, null, peliculaTriada);
	    } else {
	        seientTriat.alliberaSeient();
	        System.out.println("Pagament cancel·lat. El seient torna a estar disponible.");
	    }
	...
	boolean pagamentEntrada(BigDecimal preu) throws InterruptedException {
		boolean pagat=false;
		System.out.println("L'import a pagar es de "+preu+" €\nSi estàs conforme escriu Si [Qualsevol altra paraula cancel·la l'acció]:");
		String conforme=teclat.next();
		...
		return pagat;
			...
			System.out.println("Pagament cancel·lat.");
			return pagat=false;
		}
