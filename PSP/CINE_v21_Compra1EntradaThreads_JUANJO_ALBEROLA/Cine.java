...
/////////////////////////////////////////////////////////////////////////
	...
	//Declaració del objecte bloquejable y métode
	private static final Object teclatSincronitzat = new Object();
	
	private String llegirTeclat() {
		synchronized(teclatSincronitzat){
...
////////////////////////////////////////////////////////////////////////
	...
	public static void main(String[] args) throws InterruptedException {
		...
		do{
			...
			case 11: //Comprar ENTRADA
				Client client1=new Client(cine,"Client1");
				Client client2=new Client(cine,"Client2");
				client1.start();
				client2.start();
				client1.join();
				client2.join();
	...
	public void compraEntradaPelicula() throws InterruptedException {
	    System.out.println("["+Thread.currentThread().getName()+ "] - Comprant entrada...");
	    if (pelicules.isEmpty()) {
	        System.out.println("["+Thread.currentThread().getName()+ "] - No hi ha pel·lícules disponibles. Tornant al menú principal.");
	        return;
	    }
	    llistarPelicules();

	    // Selecció de pelicula
	    Pelicula peliculaTriada = null;
	    while (peliculaTriada == null) {
	        System.out.println("["+Thread.currentThread().getName()+ "] - Per favor, elegeix una pel·licula [0 -> Cancel·lar acció]:");
	        try {
	            int eleccio = Integer.parseInt(llegirTeclat());
	            if (eleccio == 0) return;
	            if (eleccio > 0 && eleccio <= pelicules.size()) {
	                peliculaTriada = pelicules.get(eleccio - 1);
	            } else {
	                System.out.println("["+Thread.currentThread().getName()+ "] - Opció no vàlida.");
	            }
	        } catch (NumberFormatException e) {
	            System.out.println("["+Thread.currentThread().getName()+ "] - No has introduit un valor correcte.");
	        }
	    }

	    
	    List<Sessio> sessions = peliculaTriada.getSessionsPeli();
	    if (sessions.isEmpty()) {
	        System.out.println("["+Thread.currentThread().getName()+ "] - No hi ha sessions disponibles per aquesta pel·lícula.");
	        return;
	    }
	    peliculaTriada.llistarSessionsPeli();

	    // Selecció de sessió
	    Sessio sessioTriada = null;
	    while (sessioTriada == null) {
	        System.out.println("["+Thread.currentThread().getName()+ "] - Per favor, elegeix una sessió [0 -> Cancel·lar acció]:");
	        try {
	            int eleccio = Integer.parseInt(llegirTeclat());
	            if (eleccio == 0) return;
	            if (eleccio > 0 && eleccio <= sessions.size()) {
	                sessioTriada = sessions.get(eleccio - 1);
	            } else {
	                System.out.println("["+Thread.currentThread().getName()+ "] - Opció no vàlida.");
	            }
	        } catch (NumberFormatException e) {
	    ...
	    sessioTriada.mapaSessio();

	    // Selecció del seient
	    Seient seientTriat = null;
	    while (seientTriat == null) {
	        System.out.println("[" + Thread.currentThread().getName() + "] - Tria un seient disponible [O], primer introdueix la fila [0 -> Cancel·lar acció]:");
	        try {
	            int fila = Integer.parseInt(llegirTeclat()); 
	            
	            if (fila == 0) return;

	           
	            if (fila > 0 && fila <= sessioTriada.getSeients().length) {
	                System.out.println("[" + Thread.currentThread().getName() + "] - Introdueix el nombre del seient [0 -> Cancel·lar acció]:");
	                int seient = Integer.parseInt(llegirTeclat());
	                
	                if (seient == 0) return; 
	                
	              
	                if (seient > 0 && seient <= sessioTriada.getSeients()[fila - 1].length) {
	                    seientTriat = sessioTriada.getSeients()[fila - 1][seient - 1];
	                    
	                        if (!seientTriat.verificaSeient()) {
	                            System.out.println("[" + Thread.currentThread().getName() + "] - El seient escollit no està disponible.");
	                            seientTriat = null;
	                        } else {
	                            seientTriat.reservaSeient();
	                        }
	                    
	                } else {
	                    System.out.println("[" + Thread.currentThread().getName() + "] - Número de seient invàlid.");
	                    
	                }
	            } else {
	                System.out.println("[" + Thread.currentThread().getName() + "] - Número de fila invàlid.");
	    ...
	    // Pagament de la entrada
	    
	    if (pagamentEntrada(sessioTriada.getPreu())) {
	        seientTriat.ocupaSeient();
	        System.out.println("["+Thread.currentThread().getName()+ "] - Seient ocupat amb èxit.");
	        sessioTriada.imprimirTicket(seientTriat, sessioTriada, null, peliculaTriada);
	    
	    } else {
	        seientTriat.alliberaSeient();
	        System.out.println("["+Thread.currentThread().getName()+ "] - Pagament cancel·lat. El seient torna a estar disponible.");
	    }
	...
	boolean pagamentEntrada(BigDecimal preu) throws InterruptedException {
		boolean pagat=false;
		System.out.println("["+Thread.currentThread().getName()+ "] - L'import a pagar es de "+preu+" €\nSi estàs conforme escriu Si [Qualsevol altra paraula cancel·la l'acció]:");
		String conforme=llegirTeclat();
		if(conforme.equalsIgnoreCase("Sí")||conforme.equalsIgnoreCase("Si")) {
			System.out.println("["+Thread.currentThread().getName()+"] - Pagant entrada...");
		Thread.currentThread().sleep(3000);
		pagat=true;
		return pagat;
		}else {
			System.out.println("["+Thread.currentThread().getName()+ "] -Pagament cancel·lat.");
			return pagat=false;
		}
